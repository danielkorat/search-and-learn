...
def save_dataset(dataset, config):
    if config.push_to_hub:
        ...
        dataset.to_json(
            f"{config.output_dir}/{config.approach}_seed-{config.seed}_n-{config.n}_completions.jsonl", lines=True
        )
        logger.info(
            f"Saved completions to {config.output_dir}/{config.approach}_seed-{config.seed}_n-{config.n}_completions.jsonl"
        )
